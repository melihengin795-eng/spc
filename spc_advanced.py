...
# ============================================
# AYARLAR (CLOUD UYUMLU DÜZENLEME)
# ============================================
# Klasör yolu iptal edildi, dosyalar script ile yan yana olacak
...
# ============================================
# SÜTUN EŞLEŞTİRME (İSTENEN 7 PARAMETRE)
# ============================================
COL_DATE = 'TARİH'
COL_GROUP = 'KESİT'
COL_MACHINE = 'MAKİNE'
...
# ============================================
# KAPSAMLI SPC REHBERİ
# ============================================
st.markdown('<p class="section-header">📚 SPC Kapsamlı Rehber</p>', unsafe_allow_html=True)

with st.expander("📖 SPC Nedir? - Temel Kavramlar", expanded=False):
    col1, col2 = st.columns(2)
    ...
    with col1:
        ...
        **Tanım:** Üretim veya iş süreçlerinin kalitesini izlemek ve kontrol altında 
        tutmak için istatistiksel yöntemlerin kullanılmasıdır.
        
        **Tarihçe:** 1920'lerde Walter Shewhart tarafından Bell Laboratuvarlarında geliştirilmiştir.
        
        **Temel Amaçları:**
        - 📉 **Değişkenliği Azaltmak:** Süreçteki tutarsızlıkları minimize etmek
        - ⚠️ **Erken Uyarı:** Problemleri oluşmadan tespit etmek
        - 📈 **Sürekli İyileştirme:** Kaliteyi sistematik olarak geliştirmek
        - 💰 **Maliyet Düşürme:** Fire ve yeniden işlemeyi azaltmak
        - 😊 **Müşteri Memnuniyeti:** Tutarlı ürün kalitesi sağlamak
        """)
    ...
    with col2:
        ...
        **✅ Genel Nedenler (Common Causes):**
        - Sistemin doğasında var olan rastgele değişkenlik
        - Her zaman mevcut, tahmin edilebilir sınırlar içinde
        - Düzeltmek için sistem değişikliği gerektirir
        - *Örnek:* Hammadde partileri arası küçük farklılıklar, ortam sıcaklığı
        
        **❌ Özel Nedenler (Special Causes):**
        - Normalde olmayan, tanımlanabilir değişkenlik kaynakları
        - Ara sıra ortaya çıkar, tahmin edilemez
        - Genellikle hızlıca düzeltilebilir
        - *Örnek:* Operatör hatası, arızalı alet, hatalı hammadde partisi
        
        > **💡 SPC Prensibi:** Önce özel nedenleri ortadan kaldırın (süreç "kontrol altında" olsun), 
        > sonra genel nedenleri azaltmak için sistem iyileştirmesi yapın.
        """)

with st.expander("📊 Temel İstatistikler - Formüller ve Açıklamalar", expanded=False):
    
    st.markdown("### 📈 Merkezi Eğilim ve Yayılım Ölçüleri")
    ...
    with col1:
        ...
        **Tanım:** Tüm değerlerin toplamının veri sayısına bölümü.
        ...
        **Yorumlama:**
        - Sürecin "merkez" noktasını gösterir
        - Hedef değerden sapma → ayar/kalibrasyon sorunu
        - Ortalama kayması (shift) → süreç değişimi
        
        **Örnek:** 5 ölçüm: 10.2, 10.5, 10.1, 10.4, 10.3  
        X̄ = 51.5 / 5 = **10.30**
        ...
        **Tanım:** Verilerin ortalamadan ne kadar uzaklaştığının ölçüsü.
        ...
        **Neden (n-1)?** Örneklem için Bessel düzeltmesi (daha doğru tahmin).
        
        **Yorumlama:**
        - Düşük σ = Tutarlı, stabil süreç ✅
        - Yüksek σ = Değişken, tutarsız süreç ⚠️
        """)
    
    with col2:
        st.markdown("""
        #### 📌 Moving Range (MR - Hareketli Aralık)
        
        **Tanım:** Ardışık iki ölçüm arasındaki mutlak fark.
        ...
        **Within Sigma Tahmini:**
        ```
        σ̂ = MR̄ / d₂ = MR̄ / 1.128
        ```
        
        **Not:** d₂ = 1.128, n=2 için sabittir.
        
        **Kullanım:** Kısa vadeli değişkenliği ölçer.
        
        ---
        
        #### 📌 Aralık (Range - R)
        
        **Tanım:** Veri setindeki max ve min farkı.
        ...
        **Yorumlama:**
        - Toplam yayılımı gösterir
        - Aykırı değerlerden çok etkilenir
        - Küçük örneklemlerde σ yerine kullanılabilir
        """)
    
    st.markdown("---")
    
    st.markdown("""
    ### 🔔 Normal Dağılım ve 3-Sigma Kuralı
    
    Normal dağılımda verilerin dağılımı:
    
    | Aralık | Oran | Anlamı |
    |--------|------|--------|
    | X̄ ± 1σ | %68.27 | Verilerin yaklaşık 2/3'ü |
    | X̄ ± 2σ | %95.45 | Verilerin neredeyse tamamı |
    | X̄ ± 3σ | %99.73 | Neredeyse tüm veriler |
    
    > **Sonuç:** 3σ dışına çıkma olasılığı sadece **%0.27** (1000'de 3). Bu nedenle 3σ dışındaki 
    > noktalar "özel neden" kaynaklı kabul edilir.
    """)

with st.expander("📏 Kontrol Grafikleri - UCL, LCL, Tolerans Limitleri", expanded=False):
    
    st.markdown("""
    ### 📊 Kontrol Grafiği Nedir?
    
    Süreç verilerinin zaman içindeki değişimini gösteren ve sürecin istatistiksel kontrol altında 
    olup olmadığını belirlemeye yarayan bir araçtır.
    
    **Temel Bileşenleri:**
    - 🟢 **Merkez Çizgisi (CL):** Ortalama değer (X̄)
    - 🔴 **Üst Kontrol Limiti (UCL):** X̄ + 3σ
    - 🔴 **Alt Kontrol Limiti (LCL):** X̄ - 3σ
    ...
    with col1:
        ...
        **Formül:**
        ```
        UCL = X̄ + 3σ
        UCL = X̄ + 3 × (MR̄ / 1.128)  [I-MR için]
        ```
        
        **Anlamı:** Normal koşullarda verilerin aşmaması gereken üst sınır.
        ...
        **Formül:**
        ```
        LCL = X̄ - 3σ
        LCL = X̄ - 3 × (MR̄ / 1.128)  [I-MR için]
        ```
        
        **Not:** Eğer LCL < 0 ve negatif değer mantıksızsa, LCL = 0 alınır.
        """)
    
    with col2:
        st.markdown("""
        #### ⚠️ Kontrol Limitleri vs Tolerans Limitleri
        
        | Özellik | Kontrol (UCL/LCL) | Tolerans (USL/LSL) |
        |---------|-------------------|---------------------|
        | **Kaynak** | Süreç verilerinden | Müşteri/mühendislik |
        | **Temsil** | Sürecin SESİ | Müşterinin SESİ |
        | **Amaç** | Kontrol altında mı? | Kabul edilebilir mi? |
        | **Kim belirler** | İstatistiksel hesaplama | Tasarım/müşteri |
        
        > **Kritik:** UCL ≠ USL! Birini diğerinin yerine kullanmayın!
        
        **İdeal Durum:** Kontrol limitleri tolerans limitlerinin içinde olmalı.
        """)
    
    st.markdown("""
    ---
    
    ### 📊 Bu Sistemde Kullanılan: I-MR Grafiği
    
    **I-MR (Individual - Moving Range):** Her ölçüm bireysel olarak değerlendirilir.
    
    | Grafik | Kullanım | Formül |
    |--------|----------|--------|
    | **I Grafiği** | Bireysel değerler | UCL = X̄ + 2.66×MR̄ |
    | **MR Grafiği** | Ardışık farklar | UCL = 3.267×MR̄ |
    
    **Ne Zaman Kullanılır?**
    - Tek ölçüm alındığında (n=1)
    - Pahalı veya yıkıcı testlerde
    - Homojen partilerde
    - Sürekli üretim süreçlerinde
    """)

with st.expander("⭐ Yeterlilik İndeksleri - Cp, Cpk, Pp, Ppk, PPM", expanded=False):
    
    st.markdown("""
    ### 📊 Süreç Yeterlilik İndeksleri
    
    Sürecin müşteri spesifikasyonlarını karşılama kapasitesini tek bir sayıyla ifade eder.
    """)
    ...
    with col1:
        st.markdown("""
        #### 📈 Cp (Süreç Potansiyeli)
        
        **Tanım:** Tolerans genişliğinin süreç yayılımına (6σ) oranı.
        ...
        **Neyi Ölçer:** Süreç yayılımının toleransa sığıp sığmadığı.
        
        **⚠️ Sınırlama:** Merkezlemeyi dikkate ALMAZ!
        
        ---
        
        #### ⭐ Cpk (Süreç Yeterlilik İndeksi)
        
        **Tanım:** En önemli indeks! Hem yayılımı hem merkezlemeyi dikkate alır.
        ...
        **Neyi Ölçer:** Sürecin en yakın spec limitine göre performansı.
        
        **Not:** Cpk ≤ Cp her zaman. Eşitlik sadece tam merkezde sağlanır.
        """)
    
    with col2:
        st.markdown("""
        #### 📊 Pp ve Ppk (Uzun Vadeli)
        
        **Fark:** σ yerine s (örneklem std sapması) kullanılır.
        ...
        **Cp/Cpk vs Pp/Ppk:**
        - **Cp/Cpk:** Within sigma → Kısa vadeli potansiyel
        - **Pp/Ppk:** Overall sigma → Uzun vadeli gerçek performans
        - Genellikle Pp/Ppk ≤ Cp/Cpk
        - Fark büyükse → "özel nedenler" olabilir
        
        ---
        
        #### 📉 PPM (Parts Per Million)
        
        **Tanım:** Bir milyon parçadan kaçının hatalı olacağı tahmini.
        
        **Formül:**
        ```
        PPM = [P(X > USL) + P(X < LSL)] × 10⁶
        ```
    ...
    ### 📋 Cpk Yorumlama Tablosu
    
    | Cpk Aralığı | Durum | Sigma | PPM | Yüzde Uygun | Aksiyon |
    |-------------|-------|-------|-----|-------------|---------|
    | **< 0.67** | ❌ Çok Yetersiz | < 2σ | > 45,500 | < 95.45% | 🚨 Üretimi durdur |
    | **0.67 - 1.00** | ❌ Yetersiz | 2σ - 3σ | 2,700 - 45,500 | 95.45% - 99.73% | ⚠️ %100 muayene |
    | **1.00 - 1.33** | ⚠️ Kabul Edilebilir | 3σ - 4σ | 66 - 2,700 | 99.73% - 99.99% | 📋 İyileştirme planla |
    | **1.33 - 1.67** | ✅ İyi | 4σ - 5σ | 0.6 - 66 | 99.99%+ | 👍 İzlemeye devam |
    | **1.67 - 2.00** | 🌟 Çok İyi | 5σ - 6σ | < 0.6 | 99.9999%+ | 🎯 Mükemmelliği sürdür |
    | **> 2.00** | 🏆 Mükemmel | > 6σ | < 0.002 | 99.9999998%+ | 🌟 Dünya standardı |
    
    > **Hedef:** Çoğu endüstri için minimum **Cpk ≥ 1.33** beklenir. Otomotiv sektöründe **Cpk ≥ 1.67** istenir.
    """)

with st.expander("📏 Western Electric Kuralları - Kontrol Dışı Tespiti", expanded=False):
    
    st.markdown("""
    ### 🔍 Western Electric / Nelson Kuralları
    
    Bu kurallar, kontrol grafiğinde "özel nedenlerin" varlığını tespit etmek için kullanılır.
    Bir veya daha fazla kural ihlal ediliyorsa, süreçte araştırılması gereken bir sorun var demektir.
    """)
    ...
    with col1:
        ...
        **Tespit:** Herhangi bir nokta UCL veya LCL dışında
        
        **Olasılık:** %0.27 (rastgele gerçekleşme çok düşük)
        
        **Olası Nedenler:**
        - Ölçüm/kayıt hatası
        - Hammaddede ani değişiklik
        - Makine arızası
        - Operatör hatası
        
        **Aksiyon:** Hemen araştır!
        
        ---
        
        #### 📌 Kural 2: 3 Noktadan 2'si 2σ-3σ Bölgesinde
        
        **Tespit:** 3 ardışık noktadan 2'si aynı tarafta 2σ-3σ arasında
        
        **Anlamı:** Süreç merkezden uzaklaşıyor olabilir
        
        **Olası Nedenler:**
        - Kademeli ayar kayması
        - Hammadde değişikliği
        - Çevresel değişim
        
        ---
        
        #### 📌 Kural 3: 5 Noktadan 4'ü 1σ-2σ Bölgesinde
        
        **Tespit:** 5 ardışık noktadan 4'ü aynı tarafta 1σ-2σ arasında
        
        **Olası Nedenler:**
        - Yavaş sürüklenme (drift)
        - Alet aşınması
        - Kalibrasyon kayması
        """)
    ...
    with col2:
        ...
        **Tespit:** 8+ ardışık nokta ortalamanın hep aynı tarafında
        
        **Olasılık:** (0.5)⁸ = %0.39
        
        **Anlamı:** Süreç merkezi kaymış (shift)
        
        **Olası Nedenler:**
        - Yeni operatör/ekipman
        - Yeni hammadde partisi
        - Kalıcı ayar değişikliği
        
        ---
        
        #### 📌 Kural 5: 6 Ardışık Nokta Trend
        
        **Tespit:** 6+ ardışık nokta sürekli artan veya azalan
        
        **Anlamı:** Sistematik bir değişim var
        ...
        **Olası Nedenler:**
        ...
        | Pattern | Görünüm | Olası Neden |
        |---------|---------|-------------|
        | **Shift** | Aniden yana kayma | Yeni parti/operatör |
        | **Trend** | Sürekli artış/azalış | Alet aşınması |
        | **Cyclic** | Tekrarlayan dalgalanma | Vardiya değişimi |
        | **Stratification** | Merkeze çok yakın | Yanlış limit hesabı |
        | **Mixture** | Kenarlarda yoğunlaşma | İki kaynak karışımı |
        """)

with st.expander("✅ Normallik Testi ve Varsayımlar", expanded=False):
    
    st.markdown("""
    ### 📊 Neden Normallik Testi?
    
    SPC'nin temel varsayımı, verilerin yaklaşık **normal dağılması**dır. 
    Bu varsayım sağlanmazsa, kontrol limitleri ve Cpk değerleri yanıltıcı olabilir.
    ...
    **Hipotezler:**
    - **H₀ (Null):** Veriler normal dağılımdan geliyor
    - **H₁ (Alternatif):** Veriler normal dağılımdan gelmiyor
    
    **Yorumlama:**
    - **p > 0.05:** ✅ Normal dağılım varsayımı kabul edilebilir
    - **p ≤ 0.05:** ⚠️ Normal dağılım varsayımı reddedilir
    ...
    | Ölçü | Değer | Yorum |
    |------|-------|-------|
    | **Çarpıklık (Skewness)** | = 0 | Simetrik dağılım |
    | | > 0.5 | Sağa çarpık (uzun sağ kuyruk) |
    | | < -0.5 | Sola çarpık (uzun sol kuyruk) |
    | **Basıklık (Kurtosis)** | = 0 | Normal basıklık |
    | | > 0.5 | Sivri (dar kuyruk) |
    | | < -0.5 | Basık (geniş kuyruk) |
    
    ---
    
    ### ⚠️ Normal Değilse Ne Yapmalı?
    
    1. **Box-Cox dönüşümü** uygulayın
    2. **Non-parametrik** kontrol grafikleri kullanın
    3. Verileri **alt gruplara** ayırın
    4. **Aykırı değerleri** inceleyin ve gerekirse çıkarın
    5. Veri toplama sürecini gözden geçirin
    
    > **Not:** Büyük örneklemlerde (n > 30) merkezi limit teoremi gereği, 
    > ortalamalar yine de yaklaşık normal dağılır.
    """)
...
<center>
<small>
📊 <b>Gelişmiş SPC Analiz Sistemi v3.0</b> | {SIRKET_ISMI} | 
